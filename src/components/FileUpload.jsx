...
const FileUpload = ({ onFilesUpload, maxFiles = 10, maxSize = 50 * 1024 * 1024 }) => {
  ...
  async function handleFileDrop(acceptedFiles) {
    ...
    for (const file of acceptedFiles) {
      ...
      newFiles.push({ id, file, name: file.name, size: file.size, type: file.type, status, progress, parsed });
  ...
  return (
    <Box>
      ...
      >
        <CardContent sx={{ textAlign: 'center', py: 4 }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Supported formats: Excel (.xlsx, .xls), CSV (.csv), Google Earth (.kmz, .kml)
          </Typography>
      ...
      {sheetPreview && Array.isArray(sheetPreview) && sheetPreview.length > 0 && (
        <Card sx={{ mt: 3 }}>
          <CardContent>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Typography variant="subtitle1" gutterBottom>
